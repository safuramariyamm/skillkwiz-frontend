    ...
    <div className="text-white">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button
          ...
          disabled={loading}
          className="flex items-center gap-2 bg-white/10 hover:bg-white/20 rounded-lg px-3 py-2 text-body transition-colors disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-500/20 rounded-xl p-4 text-center border border-blue-500/30">
          <p className="text-headingLg font-bold text-blue-300">{stats.total}</p>
          <p className="text-body text-gray-400 mt-1">Total Exams</p>
        </div>
        <div className="bg-green-500/20 rounded-xl p-4 text-center border border-green-500/30">
          <p className="text-headingLg font-bold text-green-300">{stats.upcoming}</p>
          <p className="text-body text-gray-400 mt-1">Upcoming</p>
        </div>
        <div className="bg-purple-500/20 rounded-xl p-4 text-center border border-purple-500/30">
          <p className="text-headingLg font-bold text-purple-300">{stats.completed}</p>
          <p className="text-body text-gray-400 mt-1">Completed</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
        {(["all", "scheduled", "completed", "cancelled"] as const).map((f) => (
          <button
            key={f}
            onClick={() => setFilter(f)}
            className={`px-4 py-2 rounded-full text-body font-medium whitespace-nowrap transition-all ${filter === f ? "bg-blue-600 text-white" : "bg-white/10 text-gray-300 hover:bg-white/20"
              }`}
          >
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400 text-body">Loading your bookings...</p>
        </div>
      ) : error ? (
        <div className="bg-red-500/20 border border-red-500/30 rounded-xl p-6 text-center">
          <XCircle className="w-10 h-10 text-red-400 mx-auto mb-3" />
          <p className="text-red-300 mb-3">{error}</p>
          <button onClick={fetchBookings} className="bg-red-500/20 hover:bg-red-500/30 text-red-300 px-4 py-2 rounded-lg text-body">
            Try Again
          </button>
        </div>
      ) : bookings.length === 0 ? (
        <div className="bg-white/5 rounded-xl p-10 text-center border border-white/10">
          ...
          <p className="text-gray-300 text-headingSm font-medium mb-2">No bookings found</p>
          <p className="text-gray-500 text-body mb-5">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      <h3 className="font-semibold capitalize text-headingSm">{booking.company}</h3>
                      <span className={`flex items-center gap-1 text-caption px-2 py-0.5 rounded-full capitalize ${statusConfig.bg} ${statusConfig.color}`}>
                        {statusConfig.icon}{booking.status}
                      </span>
                    </div>

                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-body text-gray-400">
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-3">
                      {booking.skills.map((skill) => (
                        <span key={skill} className="bg-blue-500/20 text-blue-300 text-caption px-2.5 py-0.5 rounded-full">
                          {skill}
                        </span>
                      ))}
                    </div>
