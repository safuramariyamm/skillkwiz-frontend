...
export default function AuthenticateSkillsSection() {
  ...
  return (
    <section ref={sectionRef} className="bg-[#f8fbff] sk-section-sm relative overflow-hidden">
      ...
      {/* Eyebrow + heading — stays centered in sk-container */}
      ...
      {/* Full-bleed two-column layout — breaks out of sk-container for maximum card width */}
      <div className="w-full px-4 sm:px-6 lg:px-10 xl:px-16 relative z-10">
        <div className="grid lg:grid-cols-[3fr_2fr] gap-8 lg:gap-14 items-center max-w-screen-2xl mx-auto">

          {/* Left — feature cards */}
          <div className={`transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-8"}`}>
            <div className="space-y-5">
              {features.map((f) => (
                <div key={f.title} className="sk-card p-8 flex items-start gap-7 rounded-2xl">
          ...
          <div className={`relative transition-all duration-700 delay-200 ${visible ? "opacity-100 translate-x-0" : "opacity-0 translate-x-8"}`}>
            <div className="relative h-[360px] md:h-[460px] rounded-3xl overflow-hidden shadow-2xl shadow-[#00418d]/15">
