...
export default function ServicesAuthForm({ onLogin }: ServicesAuthFormProps) {
  ...
  const sendOtp = async (identifier: string, type: "email" | "phone") => {
    ...
    try {
      ...
      if (!data.success) {
        setErrors((p) => ({ ...p, general: data.message || "Failed to send OTP" }));
        return false;
      }
      return true;
    } catch {
      setErrors((p) => ({ ...p, general: "Failed to send OTP" }));
      return false;
    } finally {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({ general: "Failed to initiate registration." });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const inputClass = (field?: string) =>
    `w-full bg-[#333333] rounded pl-10 pr-4 py-3 text-white placeholder-gray-400 focus:outline-none transition-colors ${field ? "ring-2 ring-red-400 bg-red-900/20" : "focus:ring-2 focus:ring-blue-400"}`;
  ...
  return (
    <div className="text-white">
      ...
      {success && (
        <div className="bg-green-500/20 border border-green-500/30 rounded-lg p-4 mb-4 text-center text-green-300">
      ...
      {view === "login" && (
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input type={showPassword ? "text" : "password"} value={password}
                ...
                className={`${inputClass(errors.password)} pr-10`} disabled={isLoading} required />
          ...
          <button type="submit" disabled={isLoading}
            className="w-full py-3 rounded bg-gradient-to-r from-[#4ECDC4] to-[#2d8a84] text-white hover:opacity-90 font-medium disabled:opacity-50 flex items-center justify-center gap-2">
            {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" />Signing In...</> : "Sign In"}
          </button>

          <a href={GOOGLE_AUTH_URL}
            className="w-full py-3 rounded border border-gray-500 text-white hover:bg-white/10 font-medium flex items-center justify-center gap-2 transition-all">
